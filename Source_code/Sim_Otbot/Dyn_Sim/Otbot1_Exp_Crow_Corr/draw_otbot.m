...
% Plotting the Objective trajectory
global sstates

plot(sstates(1,:),sstates(2,:),'g')

% Plotting OL trajectory CoM separated
% global OLstates
% plot(OLstates(1,:),OLstates(2,:),'-.r')

% Plotting OL trajectory CoM in Pivot Point

% global OLstatesCoMpp
% plot(OLstatesCoMpp(1,:),OLstatesCoMpp(2,:),'-.r')

% Plotting CL trajectory CoM in Pivot Point

% global CLstatesCoMpp % Using fwd_dyn
% plot(CLstatesCoMpp(1,:),CLstatesCoMpp(2,:),'-.b')

% global CLstatesCoMppdiff % Using diff with OLD CTC Tstab = 5s;
% plot(CLstatesCoMppdiff(1,:),CLstatesCoMppdiff(2,:),'-.b')

global CLstatesCoMppdiffNCTC % Using diff with NEW CTC Tstab = 3s;
plot(CLstatesCoMppdiffNCTC(1,:),CLstatesCoMppdiffNCTC(2,:),'-.b')
