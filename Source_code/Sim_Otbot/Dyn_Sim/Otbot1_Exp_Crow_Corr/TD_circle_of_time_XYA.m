...
%-------- Maxime crowded corridor trajectory using fwd_dyn --------%

% global zstatestsnew
% global pddotts
% 
% zstate = getdatasamples(resample(zstatestsnew,t),1);
% pddotval = getdatasamples(resample(pddotts,t),1);
% 
% X = zstate(1,1);
% Y = zstate(2,1);
% 
...
% Y_dot = zstate(5,1);
% 
% X_dotdot = pddotval(1,1);
% Y_dotdot = pddotval(2,1);
% 
% Alpha = zstate(3,1);
% Alpha_dot = zstate(6,1);
% Alpha_dotdot = pddotval(3,1);

%--------------------------------------------------%

%-------- Maxime crowded corridor trajectory using diff --------%

global CTCd_ts

CTC_states = getdatasamples(resample(CTCd_ts,t),1);

X = CTC_states(1,1);
Y = CTC_states(2,1);

X_dot = CTC_states(4,1);
Y_dot = CTC_states(5,1);

X_dotdot = CTC_states(7,1);
Y_dotdot = CTC_states(8,1);

Alpha = CTC_states(3,1);
Alpha_dot = CTC_states(6,1);
Alpha_dotdot = CTC_states(9,1);
