function [ xdot ] = xdot_otbot1_CTC_DF(t, xs, m, sm, u, u_Flag, cp, goal_Flag, FrictFlag, DistFlag, u_IntFlag, Int_Vec)
        ...
        switch goal_Flag
            case 'FIX'
                ...
                if strcmp(u_IntFlag,'YES')
                    I_k = Int_Vec(1,1:3)' + (t-Int_Vec(1,7))/2*(Int_Vec(1,4:6)' + pdiff);
                    u_2 = cp.K*[I_k;pdiff];
                    u = (MFIKs.')*(M_bars*u_2 + C_bars*qdot(1:3,1));        
                    qdotdot = [eye(6),zeros(6,3)]*pinv([Ms,Js.'; Js, zeros(3,3)])*[Es*u + tau_friction + D_vec - Cs*qdot; -Jdots*qdot];
                    
                    Int_Vec(1,1:3) = I_k';   % Updating Integral value
                    Int_Vec(1,4:6) = pdiff'; % Updating error value
