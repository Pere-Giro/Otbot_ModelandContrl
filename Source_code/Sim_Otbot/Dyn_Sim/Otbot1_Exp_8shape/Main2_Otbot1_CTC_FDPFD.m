...
DistFlag = 'YES'; % Flag to define if we want to add non modelled disturbances during the simulation
...
% Change the sign of the error cause is the new criterion
if strcmp(ErrorFlag,'YES') && uswitch
    figure('WindowState','maximized');
                    ...
                    for i = 1:n:length(times)
                            ...
                            if elapsed > 1200
                                ...
                                if strcmp(DistFlag,'YES') && norm(Dist_funct(times(i))) ~= 0
                                    ...
                                    D_XY = D_XY./norm(D_XY);
                                    D_XY = 3*m.l_1*D_XY;
                                    PolAng = rad2deg(atan2(-D_XY(2,1), -D_XY(1,1)));
%                                     mArrow3([states(i,1) - D_XY(1,1), states(i,2) - D_XY(2,1), 0],[states(i,1), states(i,2), 0], 'color', 'red', 'stemWidth', 3*m.l_1/50);
                                    arrows(states(i,1) - D_XY(1,1), states(i,2) - D_XY(2,1), norm(D_XY), 270 - PolAng , 'FaceColor','r','EdgeColor','none');
                                end
                                hold off
