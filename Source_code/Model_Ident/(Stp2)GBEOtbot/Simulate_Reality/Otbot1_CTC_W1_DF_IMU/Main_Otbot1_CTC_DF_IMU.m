...
FrictFlag = 'YES'; % Flag to define if we want to add friction in our model or not
...
TracePath = 'YES'; % Flag to decide how the tracking or the traces of the point will be done
...
for i=1:ls
    ...
    switch FrictFlag
        case 'YES'
            ...
            tau_friction_out(4,1) = -m.b_r*states(i,10); % tau_friction_right (only works if we are in the .m file here we must use states(i,10), and put it inside the for loop)
...
if h2>h
    ...
    y_out_data1(:,1) = states(1:hts:ls,9);
    y_out_data1(:,2:3) = IMUxy(1:hts:ls,1:2);
    ...
    savedir1 = strcat(Upath,'\Model_Identification\GBEOtbot1_V2_IMU_mcIc(Stp2)\');
    savedir2 = strcat(Upath,'\Model_Identification\GBEOtbot1_V2_IMU_mcIc(Stp2)\Final_Pars_Sim\Otbot1_CTC_W1_DF_IMU\');
